  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    lowerOpToLoops(op, operands, rewriter,
                   [loc, op](OpBuilder &builder, ValueRange memRefOperands,
                     ...
                     Value loadedLhs = binaryAdaptor.getLhs();
                     Value loadedRhs = binaryAdaptor.getRhs();

                     auto lhsType = cast<mlir::MemRefType>(binaryAdaptor.getLhs().getType());
                     if (lhsType.getShape().size() == 0) {
                       loadedLhs = builder.create<memref::LoadOp>(loc,loadedLhs);
                     } else {
                       loadedLhs = builder.create<affine::AffineLoadOp>(loc, loadedLhs, loopIvs);
                     }

                     auto rhsType = cast<mlir::MemRefType>(binaryAdaptor.getRhs().getType());
                     if (rhsType.getShape().size() == 0) {
                       loadedRhs = builder.create<memref::LoadOp>(loc,loadedRhs);
                     } else {
                       loadedRhs = builder.create<affine::AffineLoadOp>(loc, loadedRhs, loopIvs);
                     }
...
using DivOpLowering = BinaryOpLowering<toy::DivOp, arith::DivFOp>;
...
void ToyToAffineLoweringPass::runOnOperation() {
  ...
  patterns.add<AddOpLowering, ConstantOpLowering, FuncOpLowering, MulOpLowering, DivOpLowering, AndOpLowering,
